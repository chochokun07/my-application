...
(() => {
  ...
  function getVideoProjects() {
    return orderedProjects().filter((project) => project.conceptType !== "identity_prediction");
  }

  function broadcastProjectChange() {
    window.dispatchEvent(new CustomEvent("vectory:hobby-projects-change", {
      detail: { projects: getVideoProjects().map((project) => ({ ...project })) },
    }));
  }

  window.__VECTORY_HOBBY_CONTEXT__ = {
    getProjects: () => getVideoProjects().map((project) => ({ ...project })),
    getProjectById: (projectId) => getVideoProjects().find((project) => project.id === projectId) || null,
  };
  ...
  function renderProjectList() {
    ...
    const renderGroup = (status, label) => {
      ...
      return '<section class="hobby-project-group"><div class="hobby-project-group-heading"><strong>' + label + '</strong><span>' + group.length + '件</span></div><div class="hobby-project-group-list">' + group.map((project) =>
        '<button class="hobby-project-card' + (project.id === state.selectedProjectId ? " is-active" : "") + '" type="button" draggable="true" data-hobby-action="select-project" data-project-id="' + escapeHtml(project.id) + '" title="ドラッグして並び替え">' +
          ...
          '<span class="hobby-project-card-meta">' + (project.conceptType === "identity_prediction" ? "特色人格予想" : "動画プロジェクト") + " · " + PROJECT_STATUS_LABELS[project.status] + "</span></button>"
      ).join("") + '</div></section>';
    };
    container.innerHTML = renderGroup("active", "進行中のプロジェクト") + renderGroup("completed", "完了済みのプロジェクト");
  ...
  function renderProjectManageList() {
    ...
    if (!container) return;
    const renderGroup = (status, label) => {
      const group = orderedProjects().filter((project) => project.status === status);
      if (!group.length) return "";
      return '<section class="hobby-project-manage-group"><div class="hobby-project-manage-group-heading"><strong>' + label + '</strong><span>' + group.length + '件</span></div><div class="hobby-project-manage-group-list">' + group.map((project) =>
        '<div class="hobby-project-manage-row" draggable="true" data-project-id="' + escapeHtml(project.id) + '">' +
          ...
          '<span class="hobby-project-manage-actions"><label class="hobby-project-status-control">状態<select data-hobby-project-status data-project-id="' + escapeHtml(project.id) + '" aria-label="' + escapeHtml(project.name) + 'の状態"><option value="active"' + (project.status === "active" ? " selected" : "") + '>進行中</option><option value="completed"' + (project.status === "completed" ? " selected" : "") + '>完了済み</option></select></label><button class="secondary-button" type="button" data-hobby-action="edit-project" data-project-id="' + escapeHtml(project.id) + '">名称変更</button><button class="ghost-button" type="button" data-hobby-action="delete-project" data-project-id="' + escapeHtml(project.id) + '">削除</button></span>' +
        "</div>"
      ).join("") + '</div></section>';
    };
    container.innerHTML = renderGroup("active", "進行中のプロジェクト") + renderGroup("completed", "完了済みのプロジェクト");
  ...
  function renderMarkdownInline(value) {
    const links = [];
    const source = String(value ?? "").replace(/\[([^\]]+)\]\((https?:\/\/[^\s)]+)\)/g, (_match, label, url) => {
      try {
        const parsed = new URL(url);
        if (!["http:", "https:"].includes(parsed.protocol)) return label;
        const index = links.push('<a href="' + escapeHtml(parsed.href) + '" target="_blank" rel="noreferrer noopener">' + escapeHtml(label) + '</a>') - 1;
        return "\u0000link" + index + "\u0000";
      } catch (_error) {
        return label;
      }
  ...
  function renderMarkdown(markdown) {
    ...
    const flushList = () => {
      ...
      output.push("<" + tag + ">" + listItems.map((item) => "<li>" + renderMarkdownInline(item) + "</li>").join("") + "</" + tag + ">");
      listType = "";
      listItems = [];
    };
    const flushQuote = () => {
      if (!quoteLines.length) return;
      output.push('<blockquote>' + quoteLines.map(renderMarkdownInline).join("<br>") + "</blockquote>");
      quoteLines = [];
    };
    ...
    lines.forEach((line) => {
      ...
      if (list) {
        flushParagraph();
        flushQuote();
        const nextType = /^\s*\d+\.\s+/.test(line) ? "ordered" : "unordered";
    ...
    flushList();
    flushQuote();
    return output.join("") || '<p class="hobby-markdown-empty">構想はまだありません。</p>';
  }

  function renderConceptPreview() {
    const preview = $("hobbyConceptPreview");
    const body = $("hobbyConceptPreviewBody");
    const title = $("hobbyConceptPreviewTitle");
    const project = getProject();
    if (!preview || !body || !title) return;
    preview.hidden = !project;
    if (!project) return;
    title.textContent = project.name + "の構想";
    body.innerHTML = renderMarkdown(getConcept(project.id)?.body || "");
  }

  function renderConcept() {
    const project = getProject();
    const genericPanel = $("hobbyGenericConcept");
    const identityPanel = $("hobbyIdentityConcept");
    const isIdentity = project?.conceptType === "identity_prediction";
    genericPanel.hidden = !project || isIdentity;
    identityPanel.hidden = !project || !isIdentity;
    if (!project) return;
    ...
    $("hobbyIdentitySectionTabs").innerHTML = SECTIONS.map((section) => (
      '<button class="hobby-section-tab' + (section.id === state.selectedSectionId ? " is-active" : "") + '" type="button" data-hobby-section="' + escapeHtml(section.id) + '">' + escapeHtml(section.label) + "</button>"
    )).join("");

    const definitions = FIELDS[state.selectedSectionId] || [];
    const items = getIdentityItems(project.id, state.selectedTargetId, state.selectedSectionId);
    const fieldsHtml = definitions.map((field) => {
      const item = findIdentityField(project.id, state.selectedTargetId, state.selectedSectionId, field[0]);
      return '<label class="hobby-identity-field"><span>' + escapeHtml(field[1]) + "</span>" +
        '<textarea rows="4" data-identity-field="1" data-field-key="' + escapeHtml(field[0]) + '" data-field-label="' + escapeHtml(field[1]) + '" data-field-placeholder="' + escapeHtml(field[2]) + '" placeholder="' + escapeHtml(field[2]) + '">' + escapeHtml(item?.body || "") + "</textarea></label>";
    }).join("");
    const customHtml = items.filter((item) => !item.fieldKey).map((item) => (
      '<label class="hobby-identity-field hobby-custom-field"><span>' + escapeHtml(item.label) +
        ' <button class="hobby-inline-danger" type="button" data-identity-action="remove-custom" data-item-id="' + escapeHtml(item.id) + '">削除</button></span>' +
        '<textarea rows="4" data-identity-field="1" data-item-id="' + escapeHtml(item.id) + '" placeholder="' + escapeHtml(item.placeholder) + '">' + escapeHtml(item.body) + "</textarea></label>"
    )).join("");
  ...
  function renderWorkspace() {
    ...
    $("hobbyProjectStatus").textContent = PROJECT_STATUS_LABELS[project.status];
    $("hobbyProjectStatusButton").textContent = project.status === "completed" ? "進行中に戻す" : "完了にする";
  ...
  async function saveProjectOrder() {
    const rows = state.projects.map((project) => ({ id: project.id, user_id: state.user.id, name: project.name, concept_type: project.conceptType, status: project.status, output_template: project.outputTemplate, chars_per_minute: project.charsPerMinute, position: project.position, updated_at: project.updatedAt }));
    if (isRemote()) {
  ...
  function handleClick(event) {
    ...
    if (action === "open-settings" || action === "open-settings-project") openSettings("project");
      ...
      if (project) {
        ...
        saveProject(project).then(() => notify("プロジェクトの状態を更新しました")).catch((error) => notify(error.message, true));
      }
    }
  ...
  function handleChange(event) {
    ...
    if (target.matches("[data-hobby-project-status]")) {
      const project = state.projects.find((item) => item.id === target.dataset.projectId);
      if (!project) return;
      project.status = target.value === "completed" ? "completed" : "active";
      saveProject(project).then(() => notify("プロジェクトの状態を更新しました")).catch((error) => notify(error.message, true));
      return;
    }
