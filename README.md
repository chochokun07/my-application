...
- 今日／すべて／完了の表示切り替え
- タスク検索
- 作業時間の開始・休憩・再開・終了をイベントとして記録
- 作業中・休憩中の経過時間表示と、終了忘れ・長時間作業の警告
- タグ候補からのタグ選択
...
`config.js`には公開用キーだけを入れてください。`service_role` keyは絶対に入れないでください。タスク・研究プラン・研究予定・作業イベントの保護はSupabaseのRow Level Securityで行います。
...
作業時間をSupabaseと同期する場合も、最新の `supabase/schema.sql` をSQL Editorで実行して `work_events` テーブルを作成してください。SQL未実行の状態では、作業イベントはログイン中の端末にのみ保存されます。
